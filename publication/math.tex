...
\begin{definition}[Layer~2]\label{def:L2}
  The \emph{constraint landscape} (Layer~2) consists of the orientation-invariant quantities derived from the face and node structure.  For faces: frustration (the Cayley distance of holonomy from the identity), equivalently the inversion class $\{h, h^{-1}\}$.  For nodes: the conservation status ($q(v) = 1$ or $q(v) \neq 1$), equivalently the inversion class $\{q, q^{-1}\}$.  Equivalently, Layer~2 observables are functions of the relational data that are invariant under the involution $x \mapsto x^{-1}$.  Layer~2 is derived from Layer~1: given $(V, E, \phi)$, every Layer~2 quantity is computable.  Holonomy itself is \emph{not} a Layer~2 observable: its value depends on traversal direction (reversing orientation sends $\Hol(C) \mapsto \Hol(C)^{-1}$).  Similarly, specific charge values (e.g.\ $q = i$ vs.\ $q = -i$) are Layer~1 quantities: reversing all edges at a node sends $q \mapsto q^{-1}$.  What Layer~2 sees are the inversion classes, not the specific values.
\end{definition}

\begin{remark}
  Layer~2 is the quotient of the relational structure by orientation reversal.  This characterisation applies uniformly to all Layer~2 observables and is not specific to any particular quantity.
\end{remark}
...
\begin{proof}
  ...
  For Layer~2: frustration is direction-insensitive (\cref{prop:undirected}).  Conservation status ($q = 1$ or not) is direction-insensitive: reversing edges at a node sends $q \mapsto q^{-1}$, so $q = 1$ iff $q^{-1} = 1$.  Since the Layer~2 observables---frustration and conservation status---are both insensitive to edge direction, Layer~2 carries no directional information.
\end{proof}
...
\begin{example}[Frustrated triangle]\label{ex:triangle}
  ...
  \medskip
  \noindent\textbf{Layer 2} (derived constraint landscape):
  ...
  \]
  Traversing in the reverse sense gives $\Hol = (-i)^{-1} = i$.  The two values form an inversion pair in $\Zf$.
  ...
  \medskip
  \noindent\textbf{The structural split in action.}  Layer~1 carries directed information (the arrows, the distinction between $a \to b$ and $b \to a$).  Layer~2 carries undirected information (frustration~$= 1$; all nodes conserved).  This concretely illustrates \cref{thm:split}: although the underlying graph is directed, the derived frustration and conservation status are insensitive to orientation.
\end{example}
...
We have adopted frustration as the quantity mediating between Layer~2 constraints and Layer~1 growth.  Whether frustration is the unique scalar capable of playing this role---or whether other orientation-invariant quantities (e.g.\ conservation-violation density, face count) could serve as alternative or complementary drivers---remains open.
